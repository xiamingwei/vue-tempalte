        ...
        methods: {
            ...
            setPath(nodes, isRoot = true) {
                nodes.forEach(node => {
                    ...
                    if (node[this.options.recursion]) {
                        node[this.options.recursion].forEach((children, index) => {
                            this.$set(children, 'expand', true)
                            let path = node.path === 'root' ? '' : node.path
                            children.path = `${path}${index}`

                            if (children[this.options.recursion] === null) {
                                delete children[this.options.recursion]
                            }
                        })
                        this.setPath(node[this.options.recursion], false)
                    }
                })
